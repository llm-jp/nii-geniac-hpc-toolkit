...
if [[ $RUN_NCCL == 1 ]]; then
    ...
    for i in ${slurm_ids[*]}; do
        CURR_NODES=$(sed -n 6p $RESULTS_DIR/nccl.sh_${i}.log | cut -d"=" -f2) # Get the nodes in this test
        ...
        if ! [[ $CURR_BUSBW =~ $re ]]; then
            ...
            nccl_pass=0
        elif [[ $CURR_BUSBW -lt 40 ]]; then
            echo "Insufficient bus bandwidth on nodes $CURR_NODES"
            echo $CURR_NODES $CURR_BUSBW >> ${CURR_TIME}_rankings.list
            echo "See results/cluster_validation/nccl.sh_${i}.log for more details"
            nccl_pass=0
        else
            echo $CURR_NODES $CURR_BUSBW >> ${CURR_TIME}_rankings.list
